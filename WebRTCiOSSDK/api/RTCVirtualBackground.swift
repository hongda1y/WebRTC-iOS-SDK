...
@objc public class RTCVirtualBackground: NSObject {
    ...
    public override init() {
        ...
        if let metalDevice = MTLCreateSystemDefaultDevice() {
            self.ciContext = CIContext(mtlDevice: metalDevice, options: [
                ...
                .cacheIntermediates: false // MEMORY FIX: Don't cache intermediates
            ])
        } else {
    ...
    public func processAVCaptrueForegroundMask(
        ...
        processingQueue.async { [weak self] in
            ...
            // MEMORY FIX: Check if we should process this frame (skip if too frequent)
    ...
    private func updateBackgroundImageIfNeeded(_ backgroundImage: UIImage?) {
        ...
        // MEMORY FIX: Only update if image actually changed (check for actual change)
        if self.backgroundCIImage == nil {
            // MEMORY FIX: Resize background image to reasonable size to save memory
            let maxDimension: CGFloat = 1920
            let resizedImage = resizeImageIfNeeded(backgroundImage, maxDimension: maxDimension)
    ...
    private func compositeWithBlurredBackground(
        ...
        // MEMORY FIX: Clear cached blur if frame size changed significantly
        if abs(frameSize.width - lastFrameSize.width) > 10 || abs(frameSize.height - lastFrameSize.height) > 10 {
            cachedBlurredBackground = nil
        }
        
        // Cache blurred background if frame size changed
//        if frameSize != lastFrameSize || cachedBlurredBackground == nil {
            cachedBlurredBackground = self.createBlurredImage(from: backgroundImage, radius: 10)
...
//    private func compositeWithBlurredBackgroundCached(
//        foreground: CIImage?,
//        fullPixelBuffer: CVPixelBuffer,
//        frameSize: CGSize
//    ) -> CIImage {
//        let backgroundImage = CIImage(cvPixelBuffer: fullPixelBuffer)
//        
//        // Generate a simple hash of the background content
//        let backgroundHash = backgroundImage.extent.hashValue ^
//                            Int(CVPixelBufferGetDataSize(fullPixelBuffer))
//        
//        // Clear cache if frame size or content changed significantly
//        if abs(frameSize.width - lastFrameSize.width) > 10 ||
//           abs(frameSize.height - lastFrameSize.height) > 10 ||
//           backgroundHash != lastBackgroundHash {
//            cachedBlurredBackground = nil
//        }
//        
...
//           cachedBlurredBackground == nil ||
//           backgroundHash != lastBackgroundHash {
//            cachedBlurredBackground = self.createBlurredImage(from: backgroundImage, radius: 20)
//            lastFrameSize = frameSize
//            lastBackgroundHash = backgroundHash
//        }
//        
//        guard let blurredBackground = cachedBlurredBackground else {
//            return foreground ?? CIImage(color: .black)
...
//        // Check iOS version availability upfront
//        guard #available(iOS 17.0, *) else {
//            completion(nil, NSError(domain: "RTCVirtualBackground", code: -1, userInfo: [NSLocalizedDescriptionKey: "Vision framework not available"]))
//            return
//        }
//        
//        processingQueue.async { [weak self] in
...
//            guard let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) else {
//                DispatchQueue.main.async {
//                    completion(nil, NSError(domain: "RTCVirtualBackground", code: -2, userInfo: [NSLocalizedDescriptionKey: "Failed to extract CVPixelBuffer from CMSampleBuffer"]))
//                }
//                return
//            }
//            
...
//                    DispatchQueue.main.async {
//                        completion(nil, NSError(domain: "RTCVirtualBackground", code: -3, userInfo: [NSLocalizedDescriptionKey: "No person detected"]))
//                    }
//                    return
//                }
//                
...
//        }
//        
//        // Convert to pixel buffer and sample buffer
//        self.convertCIImageToPixelBuffer(finalCompositeImage, size: frameSize) { [weak self] pixelBuffer in
//            if let pixelBuffer {
//                let sampleBuffer = self?.createSampleBufferFrom(pixelBuffer: pixelBuffer,
//                                                                sampleBuffer: originalSampleBuffer)
//                completion(sampleBuffer, nil)
...
//            guard let pixelBuffer = self.convertRTCVideoFrameToPixelBuffer(videoFrame) else {
//                DispatchQueue.main.async {
//                    completion(nil, NSError(domain: "RTCVirtualBackground", code: -2, userInfo: [NSLocalizedDescriptionKey: "Failed to convert RTCVideoFrame to CVPixelBuffer"]))
//                }
//                return
//            }
//            
...
//        if self.backgroundCIImage == nil {
//            self.backgroundCIImage = CIImage(image: backgroundImage)
//        }
//    }
//    
//    private func applyForegroundMask(
...
//        self.convertCIImageToPixelBuffer(finalCompositeImage, size: frameSize) { pixelBuffer in
//            completion(pixelBuffer, nil)
//        }
//    }
//    
...
//        let centeredBackground = croppedBackground.transformed(by: CGAffineTransform(translationX: -croppedBackground.extent.origin.x, y: -croppedBackground.extent.origin.y))
//        
//        return foreground.composited(over: centeredBackground)
//    }
//    
//    private func compositeWithBackgroundForAVCapture(
//        foreground: CIImage,
...
//        }
//        
//        return foreground.composited(over: blurredBackground)
//    }
//    
//    private func applyRotationToBackground(_ background: CIImage, rotation: RTCVideoRotation) -> CIImage {
//        #if os(macOS)
...
//    }
//    
//    private func convertCIImageToPixelBuffer(
//        _ ciImage: CIImage,
//        size: CGSize,
//        completion: @escaping (CVPixelBuffer?) -> Void
//    ) {
...
//            Int(size.width),
//            Int(size.height),
//            kCVPixelFormatType_32BGRA, // Use BGRA for better Metal compatibility
//            pixelBufferAttributes as CFDictionary,
//            &pixelBuffer
//        )
//        
...
//        }
//        
//        // Render using CIContext (more efficient than CGContext for CI operations)
//        ciContext.render(ciImage, to: buffer)
//        completion(buffer)
//    }
//    
